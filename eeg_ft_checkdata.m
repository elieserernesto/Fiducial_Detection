function [data] = eeg_ft_checkdata(data, varargin)
...
elseif issource 
  data = ft_datatype_source(data);% entra aqui********* POR EL TIPO DE DATOS FUENTES
elseif isdip
...
if istrue(hascoordsys) && ~isfield(data, 'coordsys')
  data = eeg_ft_determine_coordsys(data);%   Entra aqui...!!!! modificarlo porque tengo que decirle que no pregunte si quiero cambiar no a Do you want to change the anatomical labels for the axes [Y, n]? ', 'y'!!!
end % if hascoordsys
