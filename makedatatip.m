...
elseif (~isImage && isvector(X)) || size(index,2)~=2
  hDatatip = cell(size(index)); %hDatatip = zeros(size(index));--------> 
  index = index(:);
  isLinear = true;
else
  hDatatip = cell(size(index));%hDatatip = zeros(size(index,1),1);-------->
  isLinear = false;
end
...
for n = 1:size(index,1)
  ...
  if isImage
    hDatatip{n}.Cursor.DataIndex = pos; 
    hDatatip{n}.Cursor.Position = pos(1:2);%set(get(hDatatip(n),'DataCursor'),'DataIndex',pos,...
      %'TargetPoint',pos(1:2))
  else
    %hDatatip{n}.Cursor.DataIndex = index(n,:); 
    hDatatip{n}.Cursor.Position = pos; %set(get(hDatatip(n),'DataCursor'),'DataIndex',index(n, :),...
      %'TargetPoint',pos)
  end
...
% posx = find(abs(X-24.71)<0.1);
% posy = find(abs(Y-89.5)<0.1);
% coincidencia=[];
% for i=1:length(posx)
%     for j=1:length(posy)
%         if posx(i)==posy(j)
%             coincidencia = [coincidencia;posx(i)];
%         end
%     end
% end
