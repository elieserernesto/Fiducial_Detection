function [elec] = eeg_ft_electrodeplacement_NAS(cfg, varargin)
...
switch cfg.method
  ...
  case  {'headshape', '1020'}
    headshape = eeg_fixpos(varargin{1});%From FieldTrip private... 
    headshape = eeg_ft_checkdata(headshape, 'hascoordsys', 'yes');
end
...
switch cfg.method
  case 'headshape'
    ...
    disp('Projecting Fiducials Automatically');
...
%    disp('Press "q" when you are done');
    ...
    % rotate3d on
    xyz = eeg_ft_select_point3d_NAS(headshape,'nearest', false, 'multiple', true, 'marker', '*');% ***********************************
    numelec = size(xyz, 1);
